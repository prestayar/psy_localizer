...
<html>
    <head>
        ...
        <link rel="stylesheet" href="cdn/jplayer.blue.monday.min.css" />
        <link rel="stylesheet" href="cdn/tui-image-editor.css">
        ...
        <script src="cdn/jquery-1.12.4.min.js" integrity="sha256-ZosEbRLbNQzLpnKIkEdrPv7lOy9C27hHQ+Xp8a4MxAQ=" crossorigin="anonymous"></script>
        <script src="cdn/jquery-ui.min.js" integrity="sha256-VazP97ZCwtekAsvgPBSUwPFKdrwD3unUfSGVYrahUqU=" crossorigin="anonymous"></script>
        <script src="js/plugins.js?v=<?php echo $version; ?>"></script>
        <script src="cdn/jquery.jplayer.min.js"></script>
        <link type="text/css" href="cdn/tui-color-picker.css" rel="stylesheet">
        <script type="text/javascript" src="cdn/fabric.js"></script>
        <script type="text/javascript" src="cdn/tui-code-snippet.min.js"></script>
        <script type="text/javascript" src="cdn/FileSaver.min.js"></script>
        <script type="text/javascript" src="cdn/tui-color-picker.js"></script>
        <script src="cdn/tui-image-editor.js"></script>
...
<!-- header div end -->
	...
	<div class="span12">
        ...
        foreach ($files as $file_array) {
            ...
            foreach ($files as $nu=>$file_array) {
                ...
                if(in_array($file_array['extension'], $config['ext_img'])){
                    ...
                    $src = $config['upload_dir'].$rfm_subfolder.$subdir.$file;
        ...
        if (isset($src_thumb) && $src_thumb) {
            if (($src_thumb = preg_replace('#('.addslashes($config['current_path']).')#ism', Tools::safeOutput(Context::getContext()->shop->physical_uri.'img/cms/'), $src_thumb)) == $src_thumb) {
                $src_thumb = preg_replace('#('.addslashes($config['thumbs_base_path']).')#ism', Tools::safeOutput(Context::getContext()->shop->physical_uri.'img/tmp/cms/'), $src_thumb);
            }
        }
        if (isset($mini_src) && $mini_src) {
            if (($mini_src = preg_replace('#('.addslashes($config['current_path']).')#ism', Tools::safeOutput(Context::getContext()->shop->physical_uri.'img/cms/'), $mini_src)) == $mini_src) {
                $mini_src = preg_replace('#('.addslashes($config['thumbs_base_path']).')#ism', Tools::safeOutput(Context::getContext()->shop->physical_uri.'img/tmp/cms/'), $mini_src);
            }
        }
        /* END PrestaShop */
...
?>
            ...
            <?php if($config['multiple_selection']){ ?><div class="selector">
                ...
                <figcaption>
                    ...
                    <a class="tip-right preview" title="<?php echo trans('Preview')?>" data-featherlight="<?php echo Tools::safeOutput($src);?>"  href="#"><i class=" icon-eye-open"></i></a>
